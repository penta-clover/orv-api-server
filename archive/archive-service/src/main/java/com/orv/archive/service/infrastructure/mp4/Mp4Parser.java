...
public final class Mp4Parser {
    ...
    public static long inferMoovOffsetAfterMdat(byte[] headerData, long fileSize) {
        ...
        for (Mp4BoxHeader box : boxes) {
            if ("mdat".equals(box.type())) {
                if (box.totalSize() == 0) {
                    throw new Mp4ParseException("mdat box has size=0 (extends to EOF); cannot infer moov position");
                }
    ...
    public static VideoTrackInfo parseVideoTrack(byte[] moovData) {
        ...
        int moovDataOffset = moovHeader.headerSize();

        // 2. moov 내부에 위치한 child box들을 스캔한다
        List<Mp4BoxHeader> moovChildren = scanBoxHeaders(moovData, moovDataOffset, (int) moovHeader.totalSize(), 0);

        // 3. trak box들을 찾아 비디오 트랙 식별
        for (Mp4BoxHeader trakHeader : moovChildren) {
            if (!"trak".equals(trakHeader.type())) { continue; }

            VideoTrackInfo trackInfo = parseAsVideoTrack(moovData, trakHeader);
            if (trackInfo != null) {
                return trackInfo;
            }
        }
    ...
    /**
     * trak box를 파싱하여, 비디오 트랙이면 VideoTrackInfo를 반환한다.
     */
    private static VideoTrackInfo parseAsVideoTrack(byte[] data, Mp4BoxHeader trakHeader) {
        // mdia.hdlr box를 찾아, 비디오 트랙인지 확인하기
    ...
    /**
     * 바이트 배열의 top-level box 헤더들을 순차적으로 스캔한다.
     */
    static List<Mp4BoxHeader> scanTopLevelBoxes(byte[] data) {
        return scanBoxHeaders(data, 0, data.length, 0);
    }
    ...
    private static List<Mp4BoxHeader> scanBoxHeaders(byte[] data, int start, int limit, long baseOffset) {
        ...
        while (pos + 8 <= Math.min(data.length, limit)) {
            ...
            boolean isValidHeader = header.totalSize() == 0 || header.totalSize() >= 8;
