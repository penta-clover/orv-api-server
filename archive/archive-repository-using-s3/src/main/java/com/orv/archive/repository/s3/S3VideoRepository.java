...
public class S3VideoRepository implements VideoRepository, VideoFileReader {
    ...
    public byte[] getRange(String fileKey, long offset, long length) {
        GetObjectRequest request = new GetObjectRequest(bucket, fileKey).withRange(offset, offset + length - 1);
        try (S3Object s3Object = amazonS3Client.getObject(request);
             InputStream stream = s3Object.getObjectContent()) {
            return stream.readAllBytes();
        } catch (java.io.IOException e) {
            throw new java.io.UncheckedIOException("Failed to read range [%d, %d) from %s".formatted(offset, offset + length, fileKey), e);
        }
    }
