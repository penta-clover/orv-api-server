...
이 문서는 ORV API의 각 엔드포인트에 대해 자세히 설명합니다.
요청 및 응답 예시, 사용 방법, 상태 코드 등을 확인할 수 있습니다.
모든 코드 스니펫은 Spring REST Docs를 통해 자동 생성되었습니다.
...
* 모든 API 엔드포인트는 `/api/v0/auth`로 시작합니다.
* 스니펫 파일은 `{snippets}` 속성에 지정된 경로를 기준으로 포함됩니다.
* 문서에 포함된 예시는 테스트 실행 후 자동 생성된 스니펫을 기반으로 합니다.


이 문서를 통해 ORV API의 사용법을 쉽게 이해하고, 통합 테스트와 문서화를 동시에 달성할 수 있기를 바랍니다.
