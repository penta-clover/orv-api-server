...
class JwtTokenServiceTest {
        ...
        void setUp() {
            ...
            ReflectionTestUtils.setField(jwtTokenService, "secretKey",
                    "my-very-secret-key-that-is-at-least-32-bytes-long!");
        }
        ...
        void testCreateAndValidateToken() {
            ...
            // when
            String token = jwtTokenService.createToken(subject, claims);

            // then: 토큰 생성 확인
            assertNotNull(token, "생성된 토큰은 null이 아니어야 합니다.");
            // then: 생성된 토큰이 유효해야 합니다.
            assertTrue(jwtTokenService.validateToken(token), "토큰이 유효해야 합니다.");
