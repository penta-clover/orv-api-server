...
ORV API는 소셜 로그인을 사용하여 DB 세션 기반 쿠키 인증을 지원합니다.
...
**쿼리 파라미터:** `code` (소셜 로그인 인증 코드), `state` (로그인 요청 검증 값)

이미 가입된 회원의 경우, 인증 세션을 생성한 후 프론트엔드로 리다이렉트됩니다.
...
가입되지 않은 신규 회원의 경우, 세션에 가입 대기 상태를 저장한 후 신규 가입 처리를 진행합니다.
...
소셜 인증 후 생성된 가입 대기 세션을 기반으로 회원 가입 요청을 처리하며, 성공 시 인증 세션으로 전환합니다.
