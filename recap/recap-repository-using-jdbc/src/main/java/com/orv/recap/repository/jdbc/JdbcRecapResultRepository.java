...
public class JdbcRecapResultRepository implements RecapResultRepository {
    ...
    @Override
    public Optional<RecapResultInfo> findByRecapReservationId(UUID recapReservationId) {
        ...
        RowMapper<RecapAnswerSummaryInfo> answerSummaryRowMapper = (rs, rowNum) ->
                new RecapAnswerSummaryInfo(
                        (UUID) rs.getObject("scene_id"),
                        rs.getString("question"), // Directly get 'question' from DB
                        rs.getString("summary")
                );

        List<RecapAnswerSummaryInfo> answerSummaries = jdbcTemplate.query(queryAnswerSummaries, answerSummaryRowMapper, recapResultId);

        return Optional.of(new RecapResultInfo(recapResultId, createdAt, answerSummaries));
    }
}
